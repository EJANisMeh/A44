...
local function onCombatActionActivated(player, action, ...) -- when player clicks a move button
	...
	if not playerDataFolder then
		warn("Player data folder missing for", player)
		return
	end

	local isCurrentTurn = playerDataFolder:FindFirstChild(playerData.isCurrentTurn.name)
	if not isCurrentTurn then
		warn("isCurrentTurn value missing for", player)
		return
	end
...
local function cleanupConnections(playerId)
	if script["In-CombatTest"].Value then
		print(playerId)
	end
...
local function initButtons(player)
	local playerDataFolder = player:FindFirstChild(playerData.FolderName)
...
local function destroyTextButtons(parent)
...
local function unpackOpponentsTable(opponents)
...
function IC:EndCombat(player)
	...
	if not isInCombat then
		warn("isInCombat value not found for", player)
		return
	end

	local owCanAttack = playerDataFolder:FindFirstChild(playerData.owCanAttack.name)
	if not owCanAttack then
		warn("owCanAttack value not found for", player)
		return
	end
